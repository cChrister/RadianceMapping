...
class Renderer_color(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.bias_color:
            self.mpn2 = MPN_tiny(args).to(args.device)

        self.unet = UNet(args).to(args.device)
        if args.bias_color:
            self.unet2 = UNet(args).to(args.device)
    ...
    def forward(self, colors, ray, zbufs, gt, mask_gt, isTrain, xyz_o):
        ...
        if self.alpha_blending: # [burst, 4 (self.dim), H, W]
            feature_map_zbufs = torch.cat(feature_map_zbufs, dim=0)
        else:                   # [1, burst*4 (self.dim), H, W]
        ...
        if args.bias_color:
            colors = torch.cat(color, dim=3) # [burst, H, W, 3]
            feature_map_color = colors.permute( 0, 3, 1, 2)
        else:
        ...
        if args.bias_color:
            pred_mask1 = self.mpn(feature_map_zbufs)
            pred_mask2 = self.mpn2(feature_map_color)

            fuse_rdmp1 = feature_map_zbufs * pred_mask1
            fuse_rdmp2 = feature_map_color * pred_mask2

            ret1 = self.unet(fuse_rdmp)
            ret2 = self.unet2(fuse_rdmp2)
            ret = ret1 + ret2
            img = ret.squeeze(0).permute(1, 2, 0)  # [H, W, 3]
            return {'img': img, 'gt': gt, 'mask_gt': mask_gt}
