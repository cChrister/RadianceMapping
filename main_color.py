...
from model.renderer import Renderer_color
...
if args.use_crop:
    ...
    "%y%m%d-%H%M%S-" + f'{args.expname}-color-{args.H}-crop{args.train_size}-dim{args.dim}-zbuf{args.points_per_pixel}-pix{args.pix_mask}-xyznear{args.xyznear}'))
else:
    back_path = os.path.join(log_dir, time.strftime(
    "%y%m%d-%H%M%S-" + f'{args.expname}-color-{args.H}-nocrop-dim{args.dim}-zbuf{args.points_per_pixel}-pix{args.pix_mask}-xyznear{args.xyznear}'))
