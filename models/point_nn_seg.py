...
class EncNP(nn.Module):  
    ...
    def forward(self, xyz, x):
        ...
        for i in range(self.num_stages):
            ...
            """
            i=0 x[1,288,512] xyz[1,512,3] knn_x_w[1,288,512,90]
            i=1 x[1,576,256]
            i=2 x[1,1152,128]
            i=3 x[1,2304,64]
            和分类相比最后没有池化，每一层的信息得到保留
            """
            xyz_list.append(xyz)
            x_list.append(x)
...
class Point_NN_Seg(nn.Module):
    def __init__(self, input_points=2048, num_stages=2, embed_dim=144, 
                    k_neighbors=25, de_neighbors=2, beta=1000, alpha=100):
        super().__init__()
    ...
    def forward(self, x):
        ...
        xyz = x.permute(0, 2, 1)
        print(xyz.shape[1])
        # xyz: point coordinates [1,1024,3] "debug in traindata"
        # x: point features [1,3,1024]

        # Non-Parametric Encoder
        xyz_list, x_list = self.EncNP(xyz, x) # 共4层
